...
let LocationPickerComponent = class LocationPickerComponent {
    constructor(modalCtrl, http, actionSheet, alertCtrl, geoLocation) {
        this.modalCtrl = modalCtrl;
        this.http = http;
    ...
    openMap() {
        this.modalCtrl.create({
            ...
            modalEl.onDidDismiss().then(result => {
                if (!result.data)
                    return;
                const coordinates = { lat: result.data.lat, lng: result.data.lng };
                this.createPlace(coordinates.lat, coordinates.lng);
            });
            modalEl.present();
        });
    ...
    createPlace(lat, lng) {
        ...
        this.getAddress(lat, lng).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_5__["switchMap"])(address => {
            pickedLocation.address = address;
            return Object(rxjs__WEBPACK_IMPORTED_MODULE_7__["of"])(this.getMapImage(pickedLocation.lat, pickedLocation.lng, 14));
    ...
    getAddress(lat, lng) {
        return this.http.get(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${lat},${lng}&key=${_environments_environment__WEBPACK_IMPORTED_MODULE_6__["environment"].googleMapAPIKey}`)
            .pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_5__["map"])(geoData => {
